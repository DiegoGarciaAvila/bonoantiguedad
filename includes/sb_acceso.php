        ...
        if (strlen(trim($_POST['txtnomusuario'])) > 0 && strlen(trim($_POST['txtpasswd'])) > 0 && strlen(trim($_POST['hidlogin'])) > 0) {
            if (true) {
                ...
                $datos_acceso = $consulta->executeQuery("SELECT 
                                                    ...
                                                    Where u.ClaveServidor='" . $IdPrin . "' and u.cve_estatus <> 0 and u.passwd ='" . base64_encode($pwd) . "' and p.sta_perfil <> 0 ");
             //   print_r("$datos_acceso" );die();
