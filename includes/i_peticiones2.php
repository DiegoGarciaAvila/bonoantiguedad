...
if ($str_check) {
    ...
    if ($consulta->totalRows > 0) {
       ...
       //CAMBIA EL TEXTO DEL BOTON
        ...
        $campo = array();
        $entidad = 'PETICIONES PASO 2';
        $btn_guardar='GUARDAR';
        $id_prin = 'PCveP';

        if ($__SESSION->getValueSession('desperfil') == 'ADMINISTRADOR') {
            $strWhere=" WHERE AND PPaso = 2";
               if($str_clave!==''){
                $strWhere = 'WHERE a.PCveUsufk = '.$str_clave.  "' AND PPaso = 2";
               }
        } else {
      ...
      //
       ...
       $str_impresora_destino='fichas_reporte_sistema/rep_peticiones.php?';
        ...
        $contador2= $consulta2->executeQuery("SELECT COUNT(*) as numeros FROM peticiones" );  
        ...
        switch ($_POST['opc']) {
            ...
            case 2:
               ...
               // $field[]=array('PDocGen','DOCUMENTO DE APROVACION','HIDDEN','text','NOOBLIGATORIO','varchar', 'AUN SIN GENERAR', array(0, 12),'30','',array(),'','');
                ...
                $field[]=array('PPaso','PASO','HIDDEN','number','NOOBLIGATORIO','int',  '', array(0, 12),'30',2,array(),'','');
