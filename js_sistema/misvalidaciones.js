...
function destruyedatos() {
  var encodeusuaruio =
    ...
    Base64.encode($("#txtnomusuario").val()) +
    generateRandomString(1);
  var encodeemail =
    generateRandomString(1) +
    Base64.encode($("#txtpasswd").val()) +
    generateRandomString(1);
...
function capchacorreo() {
  var encodeusuaruio =
    generateRandomString(2) +
    Base64.encode($("#txtus").val()) +
    generateRandomString(2);
  var encodeemail =
    generateRandomString(2) +
    Base64.encode($("#txtem").val()) +
    generateRandomString(2);

  var pagina = "../plantilla/login";
  $("#content").html(
    '<div class="loading"><img src="images/loader.gif" alt="VALIDANDO DATOS CONEXION LENTA" /><br/><h3><strong>VERIFICANDO DATOS...</strong></h3></div>'
  );
  $.ajax({
    url: "ajax_pass.php",
    data: "user=" + encodeusuaruio + "&email=" + encodeemail,
    type: "POST",
    dataType: "json",
    success: function (data) {
      $("#content").fadeIn(1000).html(data);
      $.each(data, function (index, element) {
        if (element.bandera == "OK") {
          alert("SE A ENVIADO EL PASSWORD AL CORREO: " + $("#txtem").val());
          location.href = pagina;
          return false;
        } else {
          alert(element.bandera);
          location.href = pagina;
          return false;
        }
      });
